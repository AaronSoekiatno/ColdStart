#!/usr/bin/env tsx
/**
 * Script to send lead campaign emails to leads (user_type='lead') via SendGrid
 * 
 * This campaign targets users who signed up for the waitlist but haven't become active users yet.
 * Goal: Encourage signup and activation.
 * 
 * Usage:
 *   npm run send-campaign:leads -- --dry-run              # Preview what would be sent
 *   npm run send-campaign:leads -- --limit=10             # Send to first 10 emails (testing)
 *   npm run send-campaign:leads                           # Send to all pending emails
 *   npm run send-campaign:leads -- --resend-failed        # Retry emails that previously failed
 */
...
// Email content - customize these for your lead campaign
const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://joinhermes.co';
const EMAIL_SUBJECT = process.env.LEAD_CAMPAIGN_SUBJECT || 'Ready to start? Your Hermes account is waiting';

// TODO: Customize this HTML template for your lead campaign
const EMAIL_HTML = process.env.LEAD_CAMPAIGN_HTML || `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
    ...
    <div style="max-width: 600px; margin: 0 auto; background-color: #1a1a2e; border-radius: 16px; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.5);">
      ...
      <div style="padding: 40px 32px;">
        <p style="font-size: 18px; line-height: 1.7; color: #f5f5f7; margin: 0 0 40px;">
          You signed up for Hermes, and we're excited to help you connect with YC founders and land your dream startup internship.
        </p>

        <p style="font-size: 16px; line-height: 1.7; color: #a1a1aa; margin: 0 0 40px;">
              ...
              <tr>
                <td style="width: 48px; vertical-align: top;" align="center">
                  <div style="width: 40px; height: 40px; background-color: rgba(212, 168, 83, 0.1); border-radius: 8px; text-align: center; line-height: 40px;">
                    <span style="color: #d4a853; font-size: 22px; line-height: 40px; display: inline-block;">1️⃣</span>
                  </div>
                </td>
                <td style="padding-left: 16px; vertical-align: top;">
                  <h3 style="margin: 0 0 4px; font-weight: 600; color: #f5f5f7; font-size: 16px;">Upload Your Resume</h3>
                  <p style="margin: 0; font-size: 14px; line-height: 1.6; color: #a1a1aa;">
                    Our AI will analyze your skills, experience, and preferences to understand what makes you unique.
                  </p>
                </td>
              </tr>
            </table>
          ...
          <div style="background-color: rgba(15, 15, 26, 0.4); border: 1px solid #2a2a4a; border-radius: 12px; padding: 20px; margin-bottom: 16px;">
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
              <tr>
                <td style="width: 48px; vertical-align: top;" align="center">
                  <div style="width: 40px; height: 40px; background-color: rgba(212, 168, 83, 0.1); border-radius: 8px; text-align: center; line-height: 40px;">
                    <span style="color: #d4a853; font-size: 22px; line-height: 40px; display: inline-block;">2️⃣</span>
                  </div>
                </td>
                <td style="padding-left: 16px; vertical-align: top;">
                  <h3 style="margin: 0 0 4px; font-weight: 600; color: #f5f5f7; font-size: 16px;">Get Matched</h3>
                  <p style="margin: 0; font-size: 14px; line-height: 1.6; color: #a1a1aa;">
                    Get matched with YC startups that align with your skills and interests.
                  </p>
                </td>
              </tr>
            </table>
          ...
          <div style="background-color: rgba(15, 15, 26, 0.4); border: 1px solid #2a2a4a; border-radius: 12px; padding: 20px;">
            <table cellpadding="0" cellspacing="0" border="0" width="100%">
              <tr>
                <td style="width: 48px; vertical-align: top;" align="center">
                  <div style="width: 40px; height: 40px; background-color: rgba(212, 168, 83, 0.1); border-radius: 8px; text-align: center; line-height: 40px;">
                    <span style="color: #d4a853; font-size: 22px; line-height: 40px; display: inline-block;">3️⃣</span>
                  </div>
                </td>
                <td style="padding-left: 16px; vertical-align: top;">
                  <h3 style="margin: 0 0 4px; font-weight: 600; color: #f5f5f7; font-size: 16px;">Generate AI-Powered Emails</h3>
                  <p style="margin: 0; font-size: 14px; line-height: 1.6; color: #a1a1aa;">
                    Generate personalized cold emails tailored to each founder and startup.
                  </p>
                </td>
              </tr>
            </table>
...
const EMAIL_TEXT = process.env.LEAD_CAMPAIGN_TEXT || `
READY TO GET STARTED?
...
3️⃣ Generate AI-Powered Emails
Generate personalized cold emails tailored to each founder and startup.
...
interface EmailPreferencesEntry {
  ...
  lead_campaign_status: 'pending' | 'sent' | 'failed' | null;
  lead_campaign_sent_at: string | null;
  lead_campaign_error: string | null;
}
...
async function main() {
  ...
  console.log('📧 Lead Campaign Email Sender (SendGrid)');
  console.log('========================================\n');
  ...
  // First, get all lead emails from waitlist table
  const { data: waitlistLeads, error: waitlistError } = await supabaseAdmin
    .from('waitlist')
    .select('email')
    .eq('user_type', 'lead');

  if (waitlistError) {
    console.error('❌ Error fetching waitlist leads:', waitlistError);
    process.exit(1);
  }

  if (!waitlistLeads || waitlistLeads.length === 0) {
    console.log('✅ No leads found in waitlist!');
    process.exit(0);
  }

  const leadEmails = waitlistLeads.map(w => w.email);
  ...
  console.log(`📊 Fetching email preferences for ${leadEmails.length} lead emails (batched into chunks of ${BATCH_SIZE})...`);

  for (let i = 0; i < leadEmails.length; i += BATCH_SIZE) {
    const emailBatch = leadEmails.slice(i, i + BATCH_SIZE);
    
    // Build query for this batch
    let query = supabaseAdmin
      .from('email_preferences')
      .select('email, unsubscribe_token, marketing_emails_enabled, unsubscribed_at, created_at, lead_campaign_status, lead_campaign_sent_at, lead_campaign_error')
      .in('email', emailBatch)
    ...
    if (resendFailed) {
      ...
      query = query.eq('lead_campaign_status', 'failed');
    } else {
      // Fetch emails that haven't been sent yet
      query = query.or('lead_campaign_status.is.null,lead_campaign_status.eq.pending');
    }
  ...
  if (!entries || entries.length === 0) {
    ...
    if (resendFailed) {
      ...
      console.log('   All lead campaign emails have already been sent.');
    }
    process.exit(0);
  }
  ...
  console.log(`📋 Found ${totalEmails} ${resendFailed ? 'failed' : 'pending'} lead campaign email(s)`);
  if (limit) {
  ...
  for (let i = 0; i < emailsToSend.length; i++) {
    ...
    await supabaseAdmin
      .from('email_preferences')
      .update({ lead_campaign_status: 'pending' })
    ...
    if (result.success) {
      ...
      const { error: updateError } = await supabaseAdmin
        .from('email_preferences')
        .update({
          lead_campaign_sent_at: new Date().toISOString(),
          lead_campaign_status: 'sent',
          lead_campaign_error: null,
        })
        .eq('email', entry.email);
      ...
      const { error: updateError } = await supabaseAdmin
        .from('email_preferences')
        .update({
          lead_campaign_status: 'failed',
          lead_campaign_error: result.error || 'Unknown error',
        })
        .eq('email', entry.email);
