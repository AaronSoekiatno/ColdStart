...
async function analyzeDataQuality() {
  console.log('🔍 Analyzing Startup Data Quality\n');
  console.log('='.repeat(70));

  // 1. Fetch all startups (pagination to handle >1000 records)
  console.log('\n📥 Fetching all startups from database...');
  ...
  // 5. Match jobs to startups and identify issues
  ...
  Object.entries(jobsByStartup).forEach(([startupKey, startupJobs]) => {
    ...
    if (!startup) {
      ...
      unknownSizeCompanyJobs.push({
        ...
        reason: 'Startup not found in startups table'
      });
      return;
    }
  ...
  if (largeCompanyJobs.length > 0) {
    ...
    console.log(`   Run: npx tsx scripts/cleanup-large-company-jobs.ts\n`);
  }
  ...
  if (unknownSizeCompanyJobs.length > 0) {
    ...
    console.log(`   Run: npx tsx scripts/enrich-team-size.ts\n`);
  }
