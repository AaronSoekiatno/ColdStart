...
export async function updateSession(sessionId, updates) {
    ...
    // If conversationHistory is not in updates, fetch the LATEST value from database
    // to avoid overwriting with stale cached data (fixes race condition)
    let conversationHistory = updates.conversationHistory;
    if (conversationHistory === undefined) {
        // Fetch latest conversation_history directly from database to avoid stale cache
        // This prevents race conditions where phase transitions might overwrite
        // conversation history that was just stored
        const { data, error } = await supabase
            .from(TABLES.SESSIONS)
        ...
        if (!error && data) {
            ...
            console.log(`[Session] Fetched fresh conversationHistory from DB: ${conversationHistory.length} messages`);
        } else {
            // Fallback to session object if DB fetch fails
            conversationHistory = session.conversationHistory || [];
            console.log(`[Session] Using session conversationHistory fallback: ${conversationHistory.length} messages`);
        }
    }

    // Merge updates, preserving the fresh conversationHistory
...
export async function recordTestResult(sessionId, testData) {
    ...
    const acceptsPassTrigger = Array.isArray(phase.transitionTrigger)
        ? phase.transitionTrigger.includes(TRANSITION_TRIGGER.PASS)
        : phase.transitionTrigger === TRANSITION_TRIGGER.PASS;
    const shouldTransition = acceptsPassTrigger && testData.conclusion === 'success';
...
export async function completeInterview(sessionId) {
    ...
    session.phaseHistory.push({
        phase: session.currentPhase || 'BUILD',
        action: 'completed',
        timestamp: now
    });
...
export async function getCurrentPhaseInfo(sessionId) {
    ...
    return {
        ...
        elapsedSeconds: phaseState.startTime
            ...
            : 0,
        remainingSeconds: phaseState.startTime && phase.duration
            ? Math.max(0, phase.duration - calculateDuration(phaseState.startTime, new Date().toISOString()))
            : phase.duration,
        durationSeconds: phaseState.startTime && phaseState.endTime
            ? calculateDuration(phaseState.startTime, phaseState.endTime)
            : null,
...
/**
 * Get phase durations for all phases in a session
 * Returns an object with phase durations in seconds
 * Useful for analytics and tracking how long each phase takes
 */
export function getPhaseDurations(session) {
    const durations = {};

    PHASE_ORDER.forEach(phaseId => {
        const phaseState = session.phases[phaseId];
        if (phaseState && phaseState.startTime) {
            const endTime = phaseState.endTime || new Date().toISOString();
            durations[phaseId] = {
                startTime: phaseState.startTime,
                endTime: phaseState.endTime || null,
                durationSeconds: calculateDuration(phaseState.startTime, endTime),
                status: phaseState.status,
                isActive: !phaseState.endTime && session.currentPhase === phaseId
            };
        } else {
            durations[phaseId] = {
                startTime: null,
                ...
                status: phaseState?.status || PHASE_STATUS.PENDING,
                isActive: false
            };
        }
    });
...
function transformSessionToDbRow(session) {
    return {
        ...
        active_vapi_call_id: session.activeVapiCallId,
        total_vapi_seconds: session.totalVapiSeconds,
        // Ensure conversation_history is always an array, never undefined/null
        // This prevents overwriting existing conversation history with null
        conversation_history: session.conversationHistory || [],
        phases: session.phases,
        phase_history: session.phaseHistory,
        updated_at: session.updatedAt
