...
// Feature flag: Set to false to disable all Vapi calls
...
export async function startInterview(candidateId, candidateData) {
    ...
    if (result.shouldActivateVapi && VAPI_ENABLED) {
    ...
    } else if (result.shouldActivateVapi && !VAPI_ENABLED) {
        console.log('[Orchestrator] Vapi is disabled via feature flag, skipping call start');
    }
...
async function handleVapiCallEnd(callData) {
    ...
    const acceptsVapiEnd = Array.isArray(phase?.transitionTrigger)
        ? phase.transitionTrigger.includes(TRANSITION_TRIGGER.VAPI_END)
        : phase?.transitionTrigger === TRANSITION_TRIGGER.VAPI_END;

    if (phase && acceptsVapiEnd) {
        try {
...
export async function transitionPhase(sessionId, trigger) {
    ...
    // Stop Vapi if active
    if (VAPI_ENABLED && vapiClient.isCallActive()) {
    ...
    // The client-side IDE page polls for phase changes and will automatically start REFLECTION calls
    if (result.shouldActivateVapi && VAPI_ENABLED) {
        ...
        console.log(`[Orchestrator] Phase transitioned to ${nextPhase.id} (Vapi required but disabled via feature flag).`);
    }
...
function startPhaseTimer(sessionId, phaseId) {
    ...
    timer.startPhaseTimer(
        ...
        async (sid, pid) => {
            ...
            // Check if this phase accepts timer-based transitions
            const phaseConfig = getPhase(pid);
            const acceptsTimer = Array.isArray(phaseConfig?.transitionTrigger)
                ? phaseConfig.transitionTrigger.includes(TRANSITION_TRIGGER.TIMER)
                : phaseConfig?.transitionTrigger === TRANSITION_TRIGGER.TIMER;

            if (!acceptsTimer) {
                console.log(`[Orchestrator] Phase ${pid} does not accept timer transitions, skipping auto-transition`);
                return;
            }
...
export async function forcePhaseTransition(sessionId, targetPhaseId) {
    ...
    if (targetPhase.vapiActive && VAPI_ENABLED) {
        try {
            await vapiClient.startPhaseCall(
                sessionId,
                targetPhaseId,
    ...
    } else if (targetPhase.vapiActive && !VAPI_ENABLED) {
        console.log(`[Orchestrator] Vapi is disabled via feature flag, skipping call start for ${targetPhaseId}`);
    }
