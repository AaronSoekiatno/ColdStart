...
export async function cancelInterviewEarly(sessionId, reason = 'user_cancelled') {
    ...
    const result = sessionManager.cancelInterview(sessionId, reason);
    result.reason = reason;

    console.log(`[Orchestrator] Interview cancelled for session ${sessionId}. cleaning up...`);
    await cleanupRepo(sessionId);
...
export async function getInterviewStatus(sessionId) {
    ...
    return {
        ...
        vapi: callStatus || { status: 'idle', active: false },
