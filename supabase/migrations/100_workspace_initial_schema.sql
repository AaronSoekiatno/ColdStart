...
BEGIN
  ...
  -- Create notifications table in candidate's schema
  EXECUTE format('
  ...
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_notifications_created_at ON %I.notifications(created_at DESC)', schema_name);
  EXECUTE format('CREATE INDEX IF NOT EXISTS idx_notifications_read ON %I.notifications(read)', schema_name);
