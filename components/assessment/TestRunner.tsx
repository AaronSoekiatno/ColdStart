...
export function TestRunner({ sessionId, className }: TestRunnerProps) {
    const [results, setResults] = useState<TestResults | null>(null);
    const [isLoading, setIsLoading] = useState(false);
    const [isOpen, setIsOpen] = useState(false);
    const { toast } = useToast();

    const runTests = async (testType: 'quick' | 'full' = 'quick') => {
        setIsLoading(true);
        // Close popover while running to minimize distraction, or keep it open?
        // User wants "only button", so maybe close it.
        setIsOpen(false);
