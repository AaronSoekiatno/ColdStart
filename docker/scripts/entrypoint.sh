...
# ============================================
# Ensure we capture ALL calls, even direct ones
ORIGINAL_CLAUDE=$(which claude 2>/dev/null || echo "/usr/local/bin/claude")
# If it's a symlink or file
...
# If headless flag detected, use our wrapper
if [[ "$*" == *"-p"* ]] || [ $# -gt 0 ]; then
